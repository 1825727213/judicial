...
//
#import "config.h"
//#define defaultWebServiceUrl @"http://ts.wzzl.cn/ElevatorXxx.asmx"
...
+(NSString*)methodSoapMessage:(NSString*)methodName{
    NSMutableString *soap=[NSMutableString stringWithFormat:@"<%@ xmlns=\"%@\">",methodName,NAMESPACE];
    [soap appendString:@"%@"];
...
+(NSString*)arrayToNameSpaceSoapMessage:(NSString*)space params:(NSArray*)arr methodName:(NSString*)methodName{
    ...
    for (NSDictionary *item in arr) {
        ...
        [msg appendFormat:@"<%@>",key];
        //[msg appendString:[item objectForKey:key]];
        [msg appendFormat:@"%@",[item objectForKey:key]];
        [msg appendFormat:@"</%@>",key];
    }
