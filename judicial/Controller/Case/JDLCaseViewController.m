...
@interface JDLCaseViewController ()<UITableViewDelegate,UITableViewDataSource,ASIHTTPRequestDelegate>
...
@implementation JDLCaseViewController
...
-(void)viewDidLayoutSubviews{
    ...
    self.tableView.frame=CGRectMake(0, y+40, self.view.frame.size.width, self.view.frame.size.height- y -40);
}
...
-(void)postCase:(NSDictionary *)dict andIntBranchIndex:(NSInteger )intBranchIndex andCaseid:(NSString *)caseid{
    ...
    [request setCompletionBlock:^{
        ...
        self.postIndex=0;
        [MBProgressHUD showMessage:[NSString stringWithFormat:@"稍等，还剩下%ld张图片",(long)self.pictures.count-self.postIndex] toView:self.view];
...
-(NSMutableArray *)cases{
    if (!_cases) {
        ...
        [_cases addObject:[JDLField initWithCaseFieldNmae:@"性别" fieldType:2 fieldContent:nil andFieldMark:@"sex" andFieldRadioValue:@[@"男",@"女"]]];
        ...
        [_cases addObject:[JDLField initWithCaseFieldNmae:@"标题" fieldType:0 fieldContent:nil andFieldMark:@"title" andFieldRadioValue:nil]];
...
-(UILabel *)describeLabel{
    if (!_describeLabel) {
        ...
        _describeLabel.text=@"在线申请法律援助前，请认真阅读首页的使用须知。通过本软件提交的法律援助申请，我们进行预审，预审通过后将会通知您携带相关资料到相关法律援助中心进行现场审核。填写内容和上传材料，请注意相关页面的提示，以保障您申请的成功率。";
