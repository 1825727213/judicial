...
-(void)setupTipTitleView{
    ...
    UIView*mask=[[UIView alloc]initWithFrame:CGRectMake(0, _maskView.sd_y+_maskView.sd_height, self.view.sd_width, height-kBorderW)];
...
- (void)setupMaskView
{
    ...
    mask.bounds = CGRectMake(0, 20, self.view.sd_width + kBorderW + kMargin , self.view.sd_width + kBorderW + kMargin);
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    if (metadataObjects.count>0) {
        ...
        if ([metadataObject.stringValue rangeOfString:@"http"].location !=NSNotFound) {
            JDLAdviceViewController * adviceVC=[[JDLAdviceViewController alloc ] init];
            adviceVC.postURL=metadataObject.stringValue;
            [self.navigationController pushViewController:adviceVC animated:YES];
        }
        else{
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"扫描结果" message: [NSString stringWithFormat:@"%@不是一个有效的案件二维码",metadataObject.stringValue]delegate:self cancelButtonTitle:@"退出" otherButtonTitles:@"再次扫描", nil];
            [alert show];
        }
    }
}
